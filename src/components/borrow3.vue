    ...
    <table class="table table-striped text-center mt-2">
        <thead>
            <tr>
                ...
                <th>Ngày mượn</th>
                <th>Ngày hẹn trả</th>
                <th>Ngày được duyệt mượn</th>
                <th>Ngày trả</th>
                <th>Ngày được duyệt trả</th>
            </tr>
        </thead>
        <tbody>
            <tr v-for="(borrow, index) in allBorrow" :key="index" >
                ...
                <td class="align-middle">{{ borrow.xacnhantra.toString().slice(0, 10) }}</td>
            </tr>
        </tbody>
    </table>
</template>
...
export default {
    ...
    methods: {
        async getAll(number) {
            try {
                const data = await BorrowService.getByUser(this.user._id, number, 'xacnhantra', '')
